...
public class SBRetroCommand extends CommandBase {
	...
	public void processCommand(ICommandSender icommandsender, String[] astring) {
		...
		if (astring.length == 1) {
			if (astring[0].equalsIgnoreCase("help")) {
				cmc.addText(String.format("[SmoothBedrock] Use '/%s %d' to cleanup the bedrock - 81 chunks.",
						getCommandName(), radius));
	...
	public boolean canCommandSenderUseCommand(ICommandSender icommandsender) {
		return super.canCommandSenderUseCommand(icommandsender)
				&& (!icommandsender.getCommandSenderName().equalsIgnoreCase("rcon") && !icommandsender
						.getCommandSenderName().equalsIgnoreCase("server"));
	}
	...
	private void setBlock(World world, int posX, int posY, int posZ, int blockId, int blockMeta, Player player) {
		...
		PacketDispatcher.sendPacketToPlayer(new SBPacketCoordinates(blockId, blockMeta, posX, posY, posZ).getPacket(),
				player);
	}
